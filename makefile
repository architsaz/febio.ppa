...
# CFLAGS = -pg -Iinclude # the run program to generate the gmon.out and open it with command gprof program gmon.out > report.txt
 ...
 LIB_DIR = lib
# LDFLAGS = -lm -lgnuplot -loperator -lmesh -lCGsolver -lCRSmat -lListDS
  LDFLAGS = -L$(MYLIB_DIR)  -lhash -lmesh -lvector -lgnuplot -loperator -lCGsolver -lCRSmat -lListDS -lm# -L$(LOC_LIB_DIR) -lListDS  # Linker flags to specify libraries
